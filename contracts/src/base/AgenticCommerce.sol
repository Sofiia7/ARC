...
import "@ozup/access/AccessControlUpgradeable.sol";
import "@ozup/proxy/utils/UUPSUpgradeable.sol";
import "@ozup/proxy/utils/Initializable.sol";
...
/// @title AgenticCommerce (Base deployment — pinned to Arc's deployed variant)
/// @notice This is NOT the current ERC-8183 reference implementation. It is a
///         byte-for-byte match (verified against ArcScan's source for
///         0x0747EEf0706327138c69792bF28Cd525089e4583, diffed against the
///         reference repo's initial commit bc9e89d, 2026-03-10) of the exact
///         "AgenticCommerce"-era variant Arc itself has deployed and never
///         upgraded. See docs/INTEGRATION_NOTES.md for the full rationale.
///
///         The current erc8183/base-contracts reference repo has since added
///         role restrictions (client != provider, evaluator != provider) that
///         make BountyAdapter's three-in-one role composition (the adapter
///         itself is client + provider + evaluator) impossible — deploying
///         *that* version would require redesigning BountyAdapter's entire
///         worker-interaction model, not just porting it to a new chain.
///         Deploying this pinned variant on Base keeps BountyAdapter.sol
///         completely unchanged, matching how it already runs against Arc's
///         own instance of the same code.
///
///         Base has no canonical ERC-8183 deployment of any kind (confirmed
///         in docs/INTEGRATION_NOTES.md), so self-deploying is the only path
///         regardless of which variant is chosen.
///
/// @dev Original source: https://eips.ethereum.org/EIPS/eip-8183#agenticcommercesol
///      (erc8183/base-contracts, MIT). platformFeeBP/evaluatorFeeBP kept at 0
///      on deploy to mirror Arc's live configuration, which BountyAdapter's
///      balance-delta payout forwarding assumes.
