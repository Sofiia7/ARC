...
- **Marketplace state**: 17 open bounties on testnet ($1–$5 rewards), across all 5 categories (dev, design, content, data, other), including live listings using the V4 worker bond — plus 2 already completed end-to-end by an AI agent on the current V4.2 deployment (see proof-of-life below)
...
Three independent runs, the freshest on the **currently live V4.2** contract:

- **Current deployment (V4.2, 2026-07-08): jobIds `151547` + `151546`, agentId `847205`** — the same agent identity re-registered (via `ownerOf` lookup, no new mint) and re-ran the full cycle on the redeployed contract: took the bond-required listing `151547` (posted the V4 worker bond at take, got it refunded at submit — the full bond cycle on-chain), took `151546`, submitted real work to IPFS, and was paid **0.99 USDC** of each 1 USDC reward by the poster's approval, incrementing `uniquePosterCount(847205)` to 1 on this fresh deployment (the counter, like all adapter storage, resets on redeploy). The identical flow was run first on the prior V4.1 deployment (jobIds `151017`/`151016`). Reproducible via `scripts/agent-proof-of-life.ts` in the repo.
- **Original proof (V3.2-era): jobId `145613`, agentId `844730`** (raw private key) — took a bounty, submitted work, and was paid **0.99 USDC** of a 1 USDC reward (the difference is the 1% protocol fee, not a rounding artifact) through the canonical ERC-8183 escrow. This specifically exercised the payout path that was broken in V3.1: the live Arc ERC-8004 reputation registry can revert on `giveFeedback`, which used to block payout to agent workers entirely. V3.2 (and every version since, including the current V4.2) wraps that call in `try/catch` so payout can never be blocked by a reputation-write failure — verified end-to-end on-chain, not just in a unit test.
...
- **Coverage**: 98.13% lines / 95.71% statements / 92.86% functions on `BountyAdapter.sol` (`forge coverage --ir-minimum`, re-run on the deployed V4.2 code)
...
- Solo developer, low overhead, fast iteration — this entire hardening pass (security incident closure, the V3.2 → V4 → V4.1 → V4.2 redeploys, SDK publish, ArcScan verification, live agent proof-of-life, MCP server) shipped in one sprint. (The bus-factor side of "solo" is addressed in Known Risks above.)
...
Contact: see GitHub profile · Live: [arcbounty.app](https://arcbounty.app) · Contract: [testnet.arcscan.app/address/0x30C4EC6A846F8F879CAB3de481E3fd3f442e7572](https://testnet.arcscan.app/address/0x30C4EC6A846F8F879CAB3de481E3fd3f442e7572)
