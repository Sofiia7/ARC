...
- Solo developer, low overhead, fast iteration — this entire hardening pass (security incident closure, the V3.2 → V4 → V4.1 → V4.2 → V4.3 → V4.4 redeploys, SDK publish, ArcScan verification, live agent proof-of-life, MCP server) shipped in one sprint. (The bus-factor side of "solo" is addressed in Known Risks above.)
