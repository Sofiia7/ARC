...
- **89 unit tests + 2 stateful invariants** (91 total; +1 fork test against live Arc Testnet = 92 with an RPC configured), **8,192 fuzz calls, 0 reverts**
- **Coverage**: 98.13% lines / 95.71% statements / 92.86% functions on `BountyAdapter.sol` (`forge coverage --ir-minimum`, re-run on the deployed V4.3 code)
